...
public class BridgeException extends Exception {
   ...
   public BridgeException(String message) {
    ...
    public BridgeException(float numberOfpoints, boolean fitWe, boolean fitThey) {
        super(!fitWe || !fitThey || numberOfpoints != MEDIUMNUMBEROFPOINS ? "Wyjątek ustawiony w złym przypadku - warunek nie jest spełniony" :
    ...
    public BridgeException(int pointDiff) {
        super(pointDiff >0 ?  "Wyjątek ustawiony w złym przypadku - warunek nie jest spełniony":
                "Błąd różnicy punktów - "+ pointDiff +" mniejsza od zera- to jakiś błąd programu");
    }
