...
public class InternationalBridgeScoring extends AbstractOneDeal {
   ...
   * @param nDRSignature                 Signature that shows is it undoubted (=1), double (=2) or redouble (=4) contract.
   ...
   *                                     <p>Number of tricks taken in game  by declarer - the player whose bid establishes the suit of the contract
  ...
  private int getSlamsBonusPoints() {
    ...
    if (oddTricks >= getContractLevel()) {
      ...
      if (getContractLevel() == 6) {
        description = description + "+ premia szlemikowa: %. ";
        return (isDeclarerVulnerable()) ? 750 : 500;
      } else if (getContractLevel() == 7) {
        description = description + "+ premia za szlema: %";
