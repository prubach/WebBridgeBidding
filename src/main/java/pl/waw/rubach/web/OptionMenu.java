...
class OptionMenu extends MenuBar {
  ...
  OptionMenu(VaadinUI ui) {
    ...
    Command commandToCalculatePoints = selectedItem -> actionOpenWindow(ui, "Okienko do liczenia punktów za serię rozdań.", actionCalculetePoints());
    ...
    Command commandToCalculatePointsRubberScorring = selectedItem -> actionOpenWindow(ui, "Okienko do liczenia punktów za jeden kontrakt.", actionCalculetePointsRubberScorring());
    optionMenuItemsCalculatePointsOld.addItem("Oblicz punkty - zapis robrowy", null, commandToCalculatePointsRubberScorring);
    Command commandToCalculatePointsOneDeal = selectedItem -> actionOpenWindow(ui, "Okienko do liczenia punktów za jeden kontrakt.", actionCalculetePointsOneDeal());
    optionMenuItemsCalculatePointsOld.addItem("Oblicz punkty jednego rozdania", null, commandToCalculatePointsOneDeal);

    Command commandToPredictWhichContractShouldBePlayed = selectedItem -> actionOpenWindow(ui, "Okienko do przeiwdywania wyników.", actionPredictsOneDealResult());
    optionMenuItemsCalculatePointsOld.addItem("Przewidywanki", null, commandToPredictWhichContractShouldBePlayed);
    Command commandToOldCalculatePoints = selectedItem -> actionOpenWindow(ui, "Stare okienko do ręcznego liczenia punktów.", actionManualCalculetePoints());
    ...
    Command commandToOpenLegend = selectedItem -> actionOpenWindow(ui, "Legenda - opis typów odzywek:", createLegendDescription());
    instructionMenuItemBidsTypes.addItem("Opis typów odzywek w okienku: ", commandToOpenLegend);
    Command commandToDisplayPointsTable = selectedItem -> actionOpenWindow(ui, "Tableki do liczenia punktów.", createPointsTable());
    instructionMenuItemBidsTypes.addItem("Tabelki  do obliczania punktów:", null, commandToDisplayPointsTable);
    Command commandToOpenDescription = selectedItem -> actionOpenWindow(ui, "Jak liczyć punkty? ", createDesciption());
    instructionMenuItemBidsTypes.addItem("Zasady obliczania punktów", null, commandToOpenDescription);
    Command commandToOpenDescriptionOfBiding = selectedItem -> actionOpenWindow(ui, "Dodatkowe informacje ", createDesciption2());
  ...
  private FormLayout actionCalculetePoints() {
    ...
    Button makeNew4GameScoring = new Button("Zacznij nowy zapis 4 rozdań", clickEvent -> setGameID(getGameID() + 1));
